...
import gamelogic.physics.AABorderShape;
...
public class EntityFactory{
  ...
  */
  public Entity createGrassTile(int x, int y){
    ...
    ret.rComp.setColor(100+colorr, 200+colorg, colorb, 255);
    ret.rComp.render();
    return ret;
  }
  ...
  */
  public Entity createEntity(int x, int y, int direction){
    ...
    if (ret.id == 0) {
      ret.rComp.setColor(Color.RED);
    } else {
      ret.rComp.setColor(Color.BLUE);
    }
    ret.rComp.render();
    return ret;
  }
  ...
  */
  public Entity createBlock(int x, int y){
    ...
    int colorg = (int)(Math.random()* 50);
    int colorb = (int)(Math.random()* 50);
    ret.rComp.setColor(colorr, colorg, colorb, 255);
    ret.rComp.render();
    return ret;
  }
  ...
  */
  public Entity createPickup(int x, int y){
    ...
    ret.rComp.setColor(230+colorr, 170+colorg, colorb, 255);
    return ret;
  }
  ...
  */
  public Entity createSnakeTail(Entity tail, boolean multi){
    ...
    if(multi){//in multiplayer, colors are assigned differently.
      ...
      colorr += ret.sComp.next.upPtr.rComp.getColor().getRed();
      colorg += ret.sComp.next.upPtr.rComp.getColor().getGreen();
      colorb += ret.sComp.next.upPtr.rComp.getColor().getBlue();
    } // end of if-else
